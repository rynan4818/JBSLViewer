...
namespace JBSLViewer.Qualifier.Tests {
    internal static class CoreScenarios {
        ...
        private static async Task ReservationGameVersion() {
            using(var r=new Rig()) {
                const string version="1.37.1_9767668645";
                r.Coordinator.GameVersion=version;
                string sent=null;
                r.Api.Result=(session,challenge,metadata,replay,key)=> {
                    sent=metadata;
                    return Task.FromResult(new ResultResponse {Status="submitted",ChallengeId=challenge});
                };
                r.Api.Reserve=(session,request,key)=> {
                    Program.Check((string)StrictJson.Object(StrictJson.Serialize(request))["gameVersion"]==version,"reserve sends actual game version including build");
                    r.Coordinator.GameVersion="changed-after-reserve";
                    return Task.FromResult(r.Api.Success(request));
                };
                r.Host.Play=context=> {
                    Program.Check(context.GameVersion==version,"reservation retains original game version");
                    return Task.FromResult(false);
                };
                await r.Coordinator.RefreshAsync();r.Coordinator.BeginConfirmation();await r.Coordinator.ConfirmAsync();
                var result=StrictJson.Object(sent);
                Program.Check((string)result["gameVersion"]==version && (string)result["endType"]=="preflight_rejected","failed standard Play result retains reservation game version");
            }
        }
        private static void ResultGameVersions() {
            using(var r=new Rig()) {
                ...
                foreach(var endType in new[]{"clear","fail"}) {
                    foreach(var version in new[]{"1.37.1","1.37.1_9767668645","1.37.1_1234567890"}) {
                ...
                foreach(var version in new string[]{null,""," \t "}) {
                    ...
                    var json=StrictJson.Object(StrictJson.Serialize(result.Metadata));
                    Program.Check((string)json["gameVersion"]=="1.37.1" && result.Metadata.ScoreValidity.InvalidReason=="preflight_rejected","preflight result supplies default for missing or blank game version");
                }
                var unobserved=QualifierResultFactory.Create(context,"preflight_rejected",null,null,r.Clock);
                Program.Check(unobserved.Metadata.GameVersion=="1.37.1" && unobserved.Metadata.ScoreValidity.PlayInstanceCount==0,"unobserved preflight result retains default game version");
                context.GameVersion="1.37.1_9767668645";
