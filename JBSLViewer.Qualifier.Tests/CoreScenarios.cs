...
namespace JBSLViewer.Qualifier.Tests {
    internal static class CoreScenarios {
        ...
        private static async Task ReservationGameVersion() {
            using(var r=new Rig()) {
                const string version="1.40.8_7379";
        ...
        private static void ResultGameVersions() {
            using(var r=new Rig()) {
                ...
                foreach(var endType in new[]{"clear","fail"}) {
                    foreach(var version in new[]{"1.40.8","1.40.8_7379","1.40.8_1234567890"}) {
                ...
                foreach(var version in new string[]{null,""," \t "}) {
                    ...
                    var json=StrictJson.Object(StrictJson.Serialize(result.Metadata));
                    Program.Check((string)json["gameVersion"]=="1.40.8" && result.Metadata.ScoreValidity.InvalidReason=="preflight_rejected","preflight result supplies default for missing or blank game version");
                }
                var unobserved=QualifierResultFactory.Create(context,"preflight_rejected",null,null,r.Clock);
                Program.Check(unobserved.Metadata.GameVersion=="1.40.8" && unobserved.Metadata.ScoreValidity.PlayInstanceCount==0,"unobserved preflight result retains default game version");
                context.GameVersion="1.40.8_7379";
