param(
    [int]$WebPort = 18380,
    [int]$ScorePort = 18381,
    [string]$MockWorkspace = (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent),
    [string]$PythonPath
)
$ErrorActionPreference = 'Stop'
$workspace = (Resolve-Path -LiteralPath $MockWorkspace).Path
if (!$PythonPath) {
    $PythonPath = @('mock_servers/score_manager/.venv/Scripts/python.exe', 'mock_servers/jbsl_web_proxy/.venv/Scripts/python.exe') |
        ForEach-Object { Join-Path $workspace $_ } | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1
}
if (!$PythonPath -or !(Test-Path -LiteralPath $PythonPath)) { throw 'Specify the mock-server venv python.exe with -PythonPath.' }
