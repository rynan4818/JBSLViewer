...
namespace JBSLViewer.Qualifier.Tests
{
    internal static class QualifierLaunchScenarios
    {
        ...
        {
            var loaded = CreateBeatmap("ExpertPlus");
            Program.Check(!new QualifierBeatmap(loaded.Key, loaded.Level, loaded.Data).IsReady,
                "a level whose exact difficulty has not been read cannot start gameplay");
            await Challenge();
            await Practice();
            await LightshowColors();
            await Preparation();
            await CallbackOrdering();
        }

        private static async Task LightshowColors()
        {
            foreach (var explicitColors in new[] { false, true })
            foreach (var overrideLightshow in new[] { false, true })
            using (var rig = new LaunchRig())
            {
                rig.Setup.colorSchemesSettings.Color = explicitColors ? new ColorScheme() : null;
                rig.Setup.colorSchemesSettings.OverrideLightshow = overrideLightshow;
                Task practice = null;
                rig.Queue(() => practice = rig.Direct.StartPracticeAsync()); await practice;
                Program.Check(rig.Engine.Calls.Count == 1
                    && rig.Engine.Calls[0].OverrideLightshow == (explicitColors || overrideLightshow),
                    "TA lightshow colors follow explicit colors or the player's override setting: " + explicitColors + "/" + overrideLightshow);
            }
        }
        ...
        private static async Task Challenge()
        {
            using (var rig = new LaunchRig())
            {
                ...
                Program.Check(call.Environments == rig.Environments && call.AdditionalInformation.backButtonText == "Menu"
                    && !call.AdditionalInformation.useTestNoteCutSoundEffects && !call.AdditionalInformation.startPaused
                    && call.AdditionalInformation.playmodeOptions == PlaymodeOptions.Default
                    && call.AdditionalInformation.recordingRelativePath == null,
                    "direct launch uses TA's standard gameplay information and installed environment model");
        ...
        private static async Task Practice()
        {
            ...
            using (var rig = new LaunchRig())
            {
                ...
                rig.Finish(state);
                Program.Check(rig.Entry.Returns.Count == 1 && rig.Entry.Returns[0].Practice && rig.Entry.Returns[0].ShowResults
                    && rig.Room.ShowingResults && rig.Room.Map.Equals(QualifierGameplayObserver.ReadMap(rig.Map.Key)), state + ": PRACTICE preserves the selected map for results");
                rig.Queue(() => practice = rig.Direct.StartPracticeAsync());
                await practice;
        ...
        private static async Task Preparation()
        {
            foreach (var change in new Action<LaunchRig>[] {
                rig => rig.Entry.SelectedBeatmap = CreateBeatmap("Expert"),
                rig => rig.Map.Level.levelID = "wrong_level",
                rig => ((TestLevelData)rig.Map.Data).Contains = false,
            ...
            {
                Task<bool> launch = null;
                rig.Queue(() => { launch = rig.StartChallenge(); rig.Room.Select(3024, QualifierGameplayObserver.ReadMap(rig.Map.Key)); });
        ...
        private static async Task CallbackOrdering()
        {
            using (var rig = new LaunchRig())
            {
                ...
                old.Restarted(new StandardLevelScenesTransitionSetupDataSO(), Result(LevelCompletionResults.LevelEndStateType.Incomplete, LevelCompletionResults.LevelEndAction.Restart));
                rig.Direct.Tick();
            ...
            using (var rig = new LaunchRig())
            {
                ...
                rig.Engine.Calls[0].Restarted(new StandardLevelScenesTransitionSetupDataSO(), Result(LevelCompletionResults.LevelEndStateType.Incomplete, LevelCompletionResults.LevelEndAction.Restart));
        ...
        private static QualifierBeatmap CreateBeatmap(string difficulty)
        {
            var level = new BeatmapLevel();
            var key = new BeatmapKey(level.levelID, new BeatmapCharacteristicSO(), (BeatmapDifficulty)Enum.Parse(typeof(BeatmapDifficulty), difficulty));
            return new QualifierBeatmap(key, level, new TestLevelData(), new BeatmapDataBasicInfo());
        }
        ...
        private sealed class LaunchRig : IDisposable
        {
            ...
            internal readonly QualifierBeatmap Map = CreateBeatmap("ExpertPlus");
            internal readonly GameplaySetupViewController Setup = new GameplaySetupViewController();
            internal readonly EnvironmentsListModel Environments = new EnvironmentsListModel();
            ...
            internal LaunchRig()
            {
                Room.Open(3023, QualifierGameplayObserver.ReadMap(Map.Key));
                Entry = new QualifierMenuEntry(Room, Map);
                Direct = new QualifierDirectPlayController(Entry, Room, Runtime, Setup, Engine, Environments);
                Adapter = new StandardPlayAdapter(Solo, Tracker, Runtime, Direct);
                Context = new ChallengeContext { Map = QualifierGameplayObserver.ReadMap(Map.Key), Submission = new SubmissionHistory() };
                Adapter.Initialize();
            }
