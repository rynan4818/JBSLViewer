...
$taskBsml = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $ModReferencesDir 'Plugins\BSML.dll'))
$taskGame = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $GameDirectory 'Beat Saber_Data\Managed\Main.dll'))
$taskViewSystemPath = Join-Path $GameDirectory 'Beat Saber_Data\Managed\BeatSaber.ViewSystem.dll'
if (-not (Test-Path -LiteralPath $taskViewSystemPath)) {
    $taskViewSystemPath = Join-Path $GameDirectory 'Beat Saber_Data\Managed\HMUI.dll'
}
$taskHmui = [Mono.Cecil.AssemblyDefinition]::ReadAssembly($taskViewSystemPath)
...
try {
    ...
    $taskOriginalMarkup = & git -c ('safe.directory=' + (Resolve-Path -LiteralPath $Repository).Path.Replace('\','/')) -C $Repository show HEAD:JBSLViewer/Views/LeaderboardMainViewController.bsml
    ...
    $taskNativeInit = $taskResultType.Methods | Where-Object { $_.Name -eq 'Init' -and $_.Parameters.Count -eq 6 }
    ...
    Check (@($taskResultCalls | Where-Object { $_.FullName -eq $taskNativeInit.FullName }).Count -eq 1) 'Dedicated results use the installed native six-argument Init API'
    ...
    $taskReport = [pscustomobject]@{
        scope='Static BSML/resources/attribution and native API/launch binding checks against installed BSML, Main.dll and the native ViewController assembly. Does not execute Unity or render VR screens.'
        viewControllerAssembly=$taskViewSystemPath
