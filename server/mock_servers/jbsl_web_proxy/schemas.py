...
def validate_projection(data: Any) -> dict[str, Any]:
    try:
        ...
        for participant in participants:
            if (not isinstance(participant, dict) or "sid" not in participant or set(participant) - {"sid", "name"}
                    or not isinstance(participant["sid"], str) or not SID_RE.fullmatch(participant["sid"])):
