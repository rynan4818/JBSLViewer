...
def schemas():
    ...
    return {
        ...
        "Leaderboard": obj({"league_id": {"type": "integer", "minimum": 1}, "league_title": text, "isLive": boolean, "isOpen": boolean, "end": date,
                            "participants": array(obj({"sid": text, "name": text})),
...
def enrich(document, role):
    ...
    document["info"]["description"] = "検証専用。Try it out はこの接続先の API に送信します。設定した障害・遅延も適用されます。"
    document["servers"] = [{"url": "/", "description": "この Swagger UI と同じ接続先"}]
    ...
    if role == "proxy":
        ...
        relay_response = response("Leaderboard")
        relay_response["description"] = "設定済みはQualifier設定を反映。未設定はJBSL-WEBのJSON本文をそのまま中継。"
        relay_response["content"]["application/json"]["schema"] = {"anyOf": [
            {"$ref": "#/components/schemas/Leaderboard"},
            {"description": "未設定リーグ: 上流が返したJSON（追加フィールドや形式を変更しません）"},
        ]}
        op.update(summary="リーグ情報を中継（設定済みはQualifierを反映）", tags=["JBSL-WEB 中継"],
                  description="未設定リーグは保存応答モードでも実JBSL-WEBへ取得します。設定済みリーグは既定でランキングのSIDを参加者へ補完します。participantsのnameはtotal_rankを優先し、名前が得られないSIDはScoreSaberから取得します。取得失敗時は以前の名前、なければSIDを返します。",
                  parameters=[param("league_text", {"type": "string", "pattern": "^[1-9][0-9]*$", "default": "3023"}, "path")],
                  responses={"200": relay_response, **error_responses})
        return document
    prefix = "/api/v1"
