...
def decode_gzip(payload: bytes, compressed_limit: int, expanded_limit: int) -> DecodedReplay:
    ...
    while r.pos < len(data):
        ...
        seen.add(section)
        if section == 0:
            r.string()  # recorder version
            game_version = r.string()
            r.string()  # client timestamp is not authoritative
            player_id = r.string()
            r.string()  # display name
            platform = r.string()
            r.string()  # tracking
            r.string()  # HMD
            r.string()  # controller
            ...
            r.f32()
            info = ReplayInfo(
                player_id, platform, song_hash.strip().upper(), difficulty, mode, score, game_version, modifiers
            )
        elif section == 1:
