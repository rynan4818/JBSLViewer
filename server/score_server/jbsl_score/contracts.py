...
def normalize_map(value: Any) -> dict[str, str]:
    ...
    if (
        not HASH_RE.fullmatch(normalized_hash)
        or not 1 <= len(characteristic) <= 100
        or normalized_difficulty not in DIFFICULTIES
    ):
...
def leaderboard_player_names(data):
    """Keep optional display metadata without making it part of participant authorization."""
    rankings = [data.get("participants"), data.get("total_rank")]
    rankings.extend(item.get("scores") for item in data["maps"])
    names = {}
    for ranking in rankings:
        ...
        for row in ranking:
            if not isinstance(row, dict):
                continue
            sid, name = row.get("sid"), row.get("name")
            if (
                isinstance(sid, str) and 1 <= len(sid) <= 128 and SID_RE.fullmatch(sid)
                and isinstance(name, str) and 1 <= len(name) <= 200 and name.strip()
                and name.strip() != sid
                and not any(ord(char) < 32 or 0xD800 <= ord(char) <= 0xDFFF for char in sid + name)
            ):
                names.setdefault(sid, name.strip())
    return names
...
def validate_projection(data: Any) -> dict[str, Any]:
    try:
        ...
        if isinstance(league_id, bool) or not isinstance(league_id, int) or not 0 < league_id <= 2147483647:
        ...
        if (
            type(qualifier["enabled"]) is not bool
            or not isinstance(qualifier["submission_method"], str)
            or not isinstance(qualifier["revision"], str)
            or not qualifier["revision"].strip()
        ):
        ...
        for participant in participants:
            if (
                ...
                or "sid" not in participant
                or set(participant) - {"sid", "name"}
                or not isinstance(participant["sid"], str)
                or not SID_RE.fullmatch(participant["sid"])
                or len(participant["sid"]) > 128
            ):
        ...
        return {
            ...
            "maps": projected_maps,
            "player_names": leaderboard_player_names(data),
        }
