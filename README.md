...
このブランチはBeat Saber **1.37.1～1.37.2**用です。`BS1.29.1`の`9c3fc19`（Qualifier機能仮搭載1）をmainの譜面APIへ移植しました。他の対象版はそれぞれ専用ブランチで扱い、版をまたぐ実行時切り替えは行いません。BSMLの参照は旧プロパティを持つ1.11系に限定します。詳細は[互換性と検証](docs/COMPATIBILITY.md)を参照してください。
