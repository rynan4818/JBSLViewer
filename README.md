...
# Qualifier（BS1.37.1専用ブランチ）

このブランチはBeat Saber **1.37.1～1.37.2**用です。`BS1.29.1`の`addb99d`（チャレンジ専用画面とTA方式の直接プレイ開始）までを各版の譜面APIへ移植しました。他の対象版はそれぞれ専用ブランチで扱い、版をまたぐ実行時切り替えは行いません。BSMLの参照は旧プロパティを持つ1.11系に限定します。今回のTA由来の変更と対象版の確認は[チャレンジ移植記録](docs/TA_CHALLENGE_PORT.md)、以前の対応範囲は[互換性と検証](docs/COMPATIBILITY.md)を参照してください。
...
予約とClear・Fail結果の`gameVersion`は実ゲームのバージョン（例：`1.37.1_9767668645`）をビルド番号ごと保持します。プレイ開始前に失敗した結果も予約時の版を保持し、録画結果ではBSORと一致する実測値を使います。旧版でバージョン不一致により`replay_mismatch`となった既存の送信停止記録は、DLL更新だけでは変更されません。
...
Visual StudioのMSBuild、.NET Framework 4.8開発ツール、およびBeat Saber 1.37.1の参照DLLが必要です。`JBSLViewer.Qualifier.Core`はこのブランチ内でModとConsoleへソースを取り込み、Core専用DLLの配布は不要です。Consoleは署名済みのNuGet版Newtonsoft.Jsonを使い、通信・認証・進行状態・Outbox・BSOR形式を検証します。
...
.\build.ps1 -GameDirectory 'C:\Program Files (x86)\Steam\steamapps\common\Beat Saber_1.37.2' `
    -ModReferencesDir 'C:\Program Files (x86)\Steam\steamapps\common\Beat Saber_1.37.1'
```
...
DLLは`JBSLViewer/bin/Release/JBSLViewer.dll`、配布ZIP・検証ログ・SHA256は`artifacts/BS1.37.1/<日時>/`へ出力します。ZIPには`Plugins/JBSLViewer.dll`と`THIRD-PARTY-NOTICES.txt`を同梱します。Modのバージョンは0.4.0です。
