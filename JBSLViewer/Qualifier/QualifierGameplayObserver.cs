...
namespace JBSLViewer.Qualifier
{
    public sealed class QualifierGameplayObserver : IInitializable, ITickable, IDisposable
    {
        ...
        public void Initialize()
        {
            ...
            try { _actualMap = ReadMap(_setup.difficultyBeatmap); }
            ...
            _context.Timing.LocalSongDurationSeconds = Finite(_setup.previewBeatmapLevel.songDuration);
            _context.Timing.SongSpeedMultiplier = Finite(_setup.gameplayModifiers.songSpeedMul);
            try
            {
                // In 1.29.1 this model is serialized on ScoreController, not bound in Zenject.
        ...
        public static MapKey ReadMap(IDifficultyBeatmap beatmap)
        {
            ...
            return MapKey.Create(levelId.Substring("custom_level_".Length),
                beatmap.parentDifficultyBeatmapSet.beatmapCharacteristic.serializedName, beatmap.difficulty.ToString());
        }
