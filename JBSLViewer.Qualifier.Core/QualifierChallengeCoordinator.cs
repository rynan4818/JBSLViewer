    ...
    public sealed class QualifierChallengeCoordinator : IDisposable {
        ...
        public async Task ConfirmAsync() {
            ...
            try {
                ...
                var context=new ChallengeContext {LeagueId=gate.LeagueId,OwnerSid=gate.CurrentSid,ScoreServerBaseUrl=gate.ScoreServerBaseUrl,ReserveKey=key,Map=gate.Map.Copy(),ClientVersion=ClientVersion,Timing=new ResultTiming {ConfirmedAtClient=_clock.UtcNow,LocalSongDurationSeconds=selection.LocalSongDurationSeconds,SongSpeedMultiplier=selection.SongSpeedMultiplier}};
                var operation=new ReserveOperation {Gate=gate,Key=key,Session=session,Context=context,Request=new ReserveRequest {LeagueId=gate.LeagueId,Map=gate.Map.Copy(),ClientVersion=ClientVersion}};
        ...
        private async Task RejectBeforePlayAsync(ChallengeContext context,string code) {
            ...
            var observed=new ResultMetadata {Diagnostics=new ResultDiagnostics {FailureCode=code}};
